...
public class GetAcknowledgeTransactionsTest {
    @Mock
    ...
    @Test
    public void getAcknowledgeTransactions_TransactionProcessed_MethodInvokedSuccesfully() throws Exception{
        ...
        testRegistry.getAcknowledgedTransactions();

        verifyZeroInteractions(mockErrorManager);
    }

    @Test
    public void getAcknowledgeTransactions_TableCantBeLoadToMemory_ThrowsCantAcknowledgeTransactionException() throws Exception{
        when(mockPluginDatabaseSystem.openDatabase(testId, IncomingCryptoDataBaseConstants.INCOMING_CRYPTO_DATABASE)).thenReturn(mockDatabase);
        when(mockDatabase.getTable(IncomingCryptoDataBaseConstants.INCOMING_CRYPTO_REGISTRY_TABLE_NAME)).thenReturn(mockTable);
